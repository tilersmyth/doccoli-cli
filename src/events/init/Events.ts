...
class InitEvents extends events.EventEmitter {
  ...
  emitter = (event: string, context: any): void => {
    this.emit("undoc_init", event, context);
  };

  start = (verbose: boolean): void => {
    this.on("undoc_init", this.action.bind(this, verbose));
  };
  ...
  stop = (): void => {
    this.spinnerReset();
    this.removeListener("undoc_init", this.action);
  };
}

export default new InitEvents();
